...
import { calculateShiftDifferential } from './logic/shiftDifferentialCalculations';
import type { ShiftDifferentialInput } from './logic/shiftDifferentialCalculations';

function App() {
  const [values, setValues] = useState<ShiftDifferentialInput>({
    baseHourlyRate: 20,
    differentialAmount: 3,
    hoursPerWeek: 40,
    weeksPerYear: 52
  });

  const handleChange = (field: keyof ShiftDifferentialInput, value: number | boolean) => {
    setValues(prev => ({ ...prev, [field]: value as number }));
  };

  const result = calculateShiftDifferential(values);
